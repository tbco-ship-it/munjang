...
Forms (key → pattern):
  ...
  n_hue (으)ㄴ 후에 · neyo 네요 · plain ㄴ/는다 · ryeogo (으)려고 해요 · giro 기로 했어요 · yagesseoyo 아/어야겠어요 · gedoeda 게 됐어요
"""
import json
...
def forms(h, pres, adj=False):
    ...
    if not adj:
        ...
        out.update({'geotgatayo': n_ending(' 것 같아요'), 'plain': h})
...
OVERRIDES = {
    '있다': {'plain': '있다', 'jeok': '있은 적이 있어요', 'juseyo': '있어 주세요', 'kkayo': '있을까요?', 'geotgatayo': '있는 것 같아요'},
    '없다': {'plain': '없다', 'geotgatayo': '없는 것 같아요', 'kkayo': '없을까요?'},
    '주다': {'juseyo': '주세요'},
    ...
    '배고프다': {'eoseo': '배고파서', 'yagesseoyo': '배고파야겠어요'},
